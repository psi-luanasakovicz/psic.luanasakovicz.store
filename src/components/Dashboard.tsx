      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <div className="lg:col-span-8 space-y-6">
          <div className="flex items-center justify-between border-b border-[#D4C6C2]/40 pb-2">
            ...
            <span className="text-xs text-[#8A645D]/70 font-semibold">
          ...
          {purchasedProducts.length > 0 ? (
            ...
            <div className="text-center py-12 bg-[#ECE9E8] rounded-2xl border border-[#D4C6C2] space-y-4">
              ...
              <p className="text-xs text-[#8A645D]/70 max-w-xs mx-auto">
                Explore nosso catálogo boutique e adquira materiais para iniciar o preenchimento.
              </p>
              <Link
                href="/materiais"
                className="inline-block bg-[#8A645D] text-white px-5 py-2.5 rounded-full text-xs font-semibold"
              >
                Ver Catálogo
              </Link>
            </div>
            ...
            {purchases.length > 0 ? (
              <div className="space-y-3">
                {purchases.map((purchase, index) => (
                  <div
                    key={purchase.id}
                    className={`flex justify-between items-center text-xs p-2.5 bg-[#F3F1F0] rounded-xl ${index > 0 ? 'opacity-75' : ''}`}
                  >
                    <div>
                      <p className="font-semibold">{purchase.productTitle}</p>
                      <p className="text-[10px] text-[#8A645D]/60">{purchase.date}</p>
                    </div>
              ...
              <p className="text-xs text-[#8A645D]/70">Nenhuma compra registrada ainda.</p>
            )}
          </div>
        </div>
      </div>
