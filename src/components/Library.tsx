...
import InteractiveAppLauncher from '@/components/InteractiveAppLauncher';
...
export default function Library({ product }: LibraryProps) {
  ...
  return (
    <div className="py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#C8DDD4]/40 pb-6">
        <div className="space-y-1 text-left">
          <Link
            href="/biblioteca"
            className="inline-flex items-center gap-1.5 text-xs font-semibold hover:opacity-85 mb-2 text-[#527A6B]/70"
          >
            <ArrowLeft className="w-3.5 h-3.5" />
            <span>Voltar para Área do Cliente</span>
          </Link>
          <h1 className="font-serif-brand text-2xl sm:text-3xl font-bold">{product.title}</h1>
          <p className="text-xs text-[#527A6B]/85 italic">{product.subtitle}</p>
        </div>

        <button
          type="button"
          onClick={() => window.print()}
          className="border border-[#C8DDD4] hover:bg-[#EEF5F2] px-4 py-2 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all self-start"
        >
          <Printer className="w-4 h-4" /> Imprimir leitura
        </button>
      </div>

      {interactiveApp && <InteractiveAppLauncher app={interactiveApp} />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <div className="lg:col-span-4 bg-[#EEF5F2] border border-[#C8DDD4] rounded-3xl p-6 space-y-4">
          <h3 className="font-serif-brand font-bold text-sm tracking-wide border-b border-[#C8DDD4]/40 pb-2">
            Conteúdo Disponível
          </h3>
          ...
          <div className="bg-[#F8FAF9] rounded-2xl p-4 text-xs space-y-2 text-[#527A6B]/80">
        ...
        <div className="lg:col-span-8 bg-[#EEF5F2] border border-[#C8DDD4] rounded-[2.5rem] p-8 md:p-12 shadow-sm text-left relative overflow-hidden">
          <div className="absolute top-0 right-0 w-32 h-32 bg-[#88B7A5]/5 rounded-full blur-2xl" />

          <div className="max-w-2xl mx-auto space-y-6">
            <div className="flex justify-between items-center text-[10px] text-[#527A6B]/50 border-b border-[#C8DDD4]/40 pb-4">
            ...
            <div className="space-y-4 min-h-[300px]">
              ...
              <p className="text-sm leading-relaxed text-[#527A6B]/95">
                {product.contents[activeSection]?.text || 'Nenhum conteúdo carregado para esta seção.'}
              </p>
            </div>

            <div className="border-t border-[#C8DDD4]/40 pt-4 flex items-center justify-between text-xs text-[#527A6B]/60">
              <span>
